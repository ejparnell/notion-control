...
export default function TaskClarifierChat({
  context,
  onTaskUpdated,
  onTasksCreated,
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const userMessage: TaskClarifierMessage = {
      role: 'user',
      content: input.trim(),
    };
    ...
    try {
      const res = await fetch('/api/task-clarifier-chat', {
        method: 'POST',
  ...
  async function handlePlanTask() {
    ...
    const userMessage: TaskClarifierMessage = {
      role: 'user',
      content: `Plan this task with ${plannerAgent}.`,
    };
    const nextMessages = [...messages, userMessage];
  ...
  async function handleApplyTaskUpdate(
    action: TaskClarifierTaskUpdateAction,
  ) {
    ...
    try {
      await updateTaskAction(context.task.id, action.patch);

      setActionStates(currentStates => ({
        ...currentStates,
        [action.id]: { status: 'applied' },
      }));
      onTaskUpdated?.(action.patch);
      router.refresh();
    } catch (err) {
      setActionStates(currentStates => ({
        ...currentStates,
  ...
  async function handleApplyTaskCreate(
    action: TaskClarifierTaskCreateAction,
  ) {
    if (!context.project) {
      return;
    }
  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-border bg-surface/80 text-foreground shadow-sm backdrop-blur">
      <div className="border-b border-border px-6 py-4">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-sm text-muted">
            {context.relatedTasks.length} related {context.relatedTasks.length === 1 ? 'task' : 'tasks'} in context
          </p>
        </div>
      </div>
      ...
      <div
        role="log"
        aria-label="Task clarifier messages"
        aria-live="polite"
            ...
            <div className="max-w-[78%] space-y-2">
              ...
              {message.role === 'assistant' && message.suggestedActions?.map(action => (
                <SuggestedActionCard
                  ...
                  onApplyTaskUpdate={handleApplyTaskUpdate}
      ...
      >
        <div className="mb-3 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center lg:justify-end">
            <label htmlFor="task-planner-agent" className="sr-only">
              Planning agent
            </label>
            <select
        ...
        <label htmlFor="task-clarifier-input" className="sr-only">
          Message
        </label>
        <textarea
          id="task-clarifier-input"
          name="message"
          rows={1}
          placeholder="Message..."
          value={input}
...
function SuggestedActionCard({
  ...
  action: TaskClarifierSuggestedAction;
  state: ActionApplyState;
  onApplyTaskUpdate: (action: TaskClarifierTaskUpdateAction) => void;
  onApplyTaskCreate: (action: TaskClarifierTaskCreateAction) => void;
}) {
  ...
  return (
    <TaskUpdateActionCard
      action={action}
      state={state}
      onApply={onApplyTaskUpdate}
    />
  );
}

function TaskUpdateActionCard({
  action,
  state,
  onApply,
}: {
  action: TaskClarifierTaskUpdateAction;
  state: ActionApplyState;
  onApply: (action: TaskClarifierTaskUpdateAction) => void;
}) {
  const disabled =
  ...
  return (
    <div className="rounded-xl border border-primary/40 bg-primary-soft px-3 py-3 text-sm text-foreground shadow-sm">
      ...
      <TaskPlanDetails task={action.patch} />
...
function TaskCreateActionCard({
  ...
  action: TaskClarifierTaskCreateAction;
  state: ActionApplyState;
  onApply: (action: TaskClarifierTaskCreateAction) => void;
}) {
  const disabled =
    state.status === 'applying' ||
    state.status === 'applied' ||
...
function formatTaskCreateDetails(
  task: TaskClarifierTaskCreateAction['tasks'][number],
) {
  const details = [
    task.status,
    task.priority,
...
function TaskPlanDetails({
  ...
  task: Pick<
    TaskClarifierTaskUpdatePatch,
    'description' | 'acceptanceCriteria' | 'testingCriteria' | 'implementationPlan'
  >;
}) {
  const hasDetails =
    Boolean(task.description?.trim()) ||
    Boolean(task.acceptanceCriteria?.length) ||
    Boolean(task.testingCriteria?.length) ||
    Boolean(task.implementationPlan?.length);
