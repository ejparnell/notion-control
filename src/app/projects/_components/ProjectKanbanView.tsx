...
import { PriorityPill, StatusPill } from '@/components/shared/data-display/TableCells';
...
type ProjectKanbanViewProps = {
  ...
  onMoveStatus: (move: ProjectKanbanMove) => void | Promise<void>;
};
...
const columns: KanbanColumn[] = statusValues.map(status => ({
  ...
  emptyMessage: 'Drop projects here.',
}));
...
function getProjectStatus(project: ProjectInterface) {
  return project.status ?? STATUS_MAP.BACKLOG.name;
}
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <Link
          ...
          className="min-w-0 flex-1 font-semibold text-foreground transition-colors hover:text-primary"
        >
      ...
      <div className="mt-3 flex flex-wrap gap-1.5">
        <StatusPill value={project.status ?? null} />
        <PriorityPill value={project.priority ?? null} />
      </div>

      {(project.startDate || project.endDate || project.estTime != null) && (
        <div className="mt-3 grid gap-1.5 text-xs text-muted-soft">
          ...
          {project.estTime != null && (
            <span>{project.estTime}h estimated</span>
          )}
        </div>
      )}
...
export default function ProjectKanbanView({
  ...
  async function handleMove(event: KanbanMoveEvent<ProjectInterface>) {
  ...
  return (
    <KanbanView
      items={projects}
      columns={columns}
      getColumnId={getProjectStatus}
      getItemLabel={project => project.name}
      emptyMessage="No projects."
      onMove={handleMove}
      renderCard={(project, state) => (
        <ProjectKanbanCard
          project={project}
